import Reveal from "@/components/Reveal";
import SectionHeading from "./SectionHeading";
import type { IconItem, TitlePart } from "./clusterKit";

/**
 * Two-column section: a primary list of icon + label + detail rows on the
 * left, an optional highlighted aside on the right. Dark band
 * (bg-grain). Used for "when it's the right call" / "signs you need
 * this" / "what counts as enterprise".
 */
export default function SplitFeature({
  id,
  eyebrow,
  titleParts,
  intro,
  items,
  aside,
}: {
  id: string;
  eyebrow: string;
  titleParts: TitlePart[];
  intro?: string;
  items: IconItem[];
  aside?: { heading: string; points: IconItem[] };
}) {
  return (
    <section
      id={id}
      className="bg-grain relative scroll-mt-24 overflow-hidden bg-ink text-paper"
    >
      <div
        aria-hidden
        className="pointer-events-none absolute top-[-12%] left-[-10%] h-[460px] w-[460px] rounded-full bg-signal/15 blur-[140px] animate-ambient-drift"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,theme(colors.paper/4)_1px,transparent_1px),linear-gradient(to_bottom,theme(colors.paper/4)_1px,transparent_1px)] bg-[size:56px_56px]"
      />

      <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-32">
        <SectionHeading
          eyebrow={eyebrow}
          titleParts={titleParts}
          intro={intro}
          tone="dark"
        />

        <div
          className={`mt-16 grid gap-12 lg:gap-16 ${
            aside ? "lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)]" : ""
          }`}
        >
          <ul className="space-y-3">
            {items.map((item, i) => {
              const Icon = item.icon;
              return (
                <Reveal
                  key={item.name}
                  as="li"
                  delay={i * 0.06}
                  className="flex gap-4 rounded-sm border border-paper/10 bg-graphite/30 p-4 transition-colors hover:border-signal/45"
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-paper/10 bg-paper/[0.04]">
                    <Icon className="h-5 w-5 text-signal" strokeWidth={1.75} />
                  </span>
                  <div>
                    <p className="font-display text-sm font-semibold text-paper sm:text-base">
                      {item.name}
                    </p>
                    <p className="mt-0.5 text-xs leading-relaxed text-paper/60 sm:text-sm">
                      {item.detail}
                    </p>
                  </div>
                </Reveal>
              );
            })}
          </ul>

          {aside && (
            <div>
              <h3 className="font-mono text-xs font-semibold uppercase tracking-widest text-paper/50">
                {aside.heading}
              </h3>
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                {aside.points.map((point, i) => {
                  const Icon = point.icon;
                  return (
                    <Reveal
                      key={point.name}
                      delay={i * 0.05}
                      className="rounded-sm border border-ember/20 bg-ember/[0.06] p-4"
                    >
                      <div className="flex items-center gap-2.5">
                        <Icon
                          className="h-4 w-4 shrink-0 text-ember"
                          strokeWidth={1.75}
                        />
                        <p className="font-display text-sm font-semibold text-paper">
                          {point.name}
                        </p>
                      </div>
                      <p className="mt-1.5 text-xs leading-relaxed text-paper/60">
                        {point.detail}
                      </p>
                    </Reveal>
                  );
                })}
              </div>
            </div>
          )}
        </div>
      </div>
    </section>
  );
}
