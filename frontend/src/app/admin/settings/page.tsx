...
export default function AdminSettingsPage() {
  ...
  const [bookConsultationHeroImageUrl, setBookConsultationHeroImageUrl] = useState("");
  ...
  const [uploadingBookConsultationHero, setUploadingBookConsultationHero] = useState(false);
    ...
    const load = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          setBookConsultationHeroImageUrl(data.bookConsultationHeroImageUrl);
  ...
  const handleBookConsultationHeroFileChange = async (
    ...
    setUploadingBookConsultationHero(true);
    setError("");
    setSuccess(false);
    try {
      const url = await uploadImage(file);
      setBookConsultationHeroImageUrl(url);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to upload consultation hero image."
      );
    } finally {
      setUploadingBookConsultationHero(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="admin-fade-in mt-8 grid grid-cols-1 gap-6 rounded-xl border border-graphite/10 bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        <div className="md:col-span-2">
          <label className={labelClass}>Book Consultation Hero Background</label>
          <p className="mt-1 text-xs text-graphite/50">
            The image behind the &quot;Book a free consultation&quot; title on
            /book-consultation. Leave unset for the dark grid backdrop.
          </p>
          <div className="mt-3 flex h-40 w-full items-center justify-center overflow-hidden rounded-lg border border-dashed border-graphite/15 bg-graphite/5">
            {bookConsultationHeroImageUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={resolveImageUrl(bookConsultationHeroImageUrl)}
                alt="Consultation hero preview"
                className="h-full w-full object-cover"
              />
            ) : (
              <div className="flex flex-col items-center gap-2 text-graphite/30">
                <ImageIcon className="h-6 w-6" />
                <span className="text-xs">No consultation hero image set</span>
              </div>
            )}
          </div>

          <label className="mt-3 flex w-fit cursor-pointer items-center gap-2 rounded-lg border border-graphite/15 px-4 py-2.5 text-sm font-medium text-graphite transition hover:border-signal hover:text-signal">
            {uploadingBookConsultationHero ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <Upload className="h-4 w-4" />
            )}
            {uploadingBookConsultationHero
              ? "Uploading..."
              : "Upload consultation hero image"}
            <input
              type="file"
              accept="image/jpeg,image/png,image/webp,image/gif,image/svg+xml"
              className="hidden"
              onChange={handleBookConsultationHeroFileChange}
              disabled={uploadingBookConsultationHero}
            />
          </label>
        </div>
